...
with graph.as_default():
    tf_train_dataset = tf.constant(train_dataset[:train_subdata])
    tf_train_labels = tf.constant(train_labels[:train_subdata])
...
with tf.Session(graph=graph) as s:
    ...
    for step in range(num_train):
        _,l,predictions = s.run([optimizer,loss,train_prediction])
        if(step%100 == 0):
            print('Training accuracy is : {}'.format(accuracy(predictions,train_labels[:train_subdata,:])))
